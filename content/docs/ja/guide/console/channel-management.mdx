...
ここでは、MAX API のアップストリームチャネルを管理できます。
...
この設定は **DoubaoVideo** チャネルでのみ利用できます。チャネルを編集し、高度な設定を展開して **Video Task Billing** を開くと、**Completion delta settlement** を制御できます。

- 既定では有効です。タスク送信時にこのスイッチのスナップショットが保存され、完了時には上流が返す `usage.total_tokens` を使って、予約済みクォータへの差額請求または返金を行えます。
- 上流 usage が安定しない、チャネル料金が未調整、または送信済みタスクを完了後に再調整したくない場合は無効にします。無効時は送信時点の予約または固定の課金結果が維持され、完了時の自動請求・返金は行われません。
- このスイッチは DoubaoVideo にのみ適用されます。ほかの動画チャネルは `disable_task_delta_settlement` を無視します。設定はチャネルの `settings` JSON に保存され、データベース移行は不要です。

この設定は新しく送信したタスクに保存されます。スナップショットを持たない未完了の旧タスクは、現在のチャネル設定を参照します。このスイッチにかかわらず、実際の上流コストに合わせて `ModelRatio` とタスク課金設定を維持してください。
