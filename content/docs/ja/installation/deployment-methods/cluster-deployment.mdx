...
このドキュメントでは、MAX API クラスターデプロイの詳細な設定手順とベストプラクティスを提供し、高可用性で負荷分散された分散システムを構築するのに役立ちます。
...
MAX API クラスターはマスター・スレーブアーキテクチャ設計を採用しています。
...
services:
  max-api-master:
    ...
    environment:
      ...
      # - FRONTEND_BASE_URL=https://<your-max-api-domain> # フロントエンドのベースURL（メール通知機能などに使用）
    volumes:
      - ./data:/data
      - ./logs:/app/logs
```
...
services:
  max-api-slave:
    ...
    environment:
      - SQL_DSN=root:password@tcp(your-db-host:3306)/max-api # マスターノードと同じ
      ...
      - TZ=Asia/Shanghai
      # 以下はオプション設定
      # - FRONTEND_BASE_URL=https://<your-max-api-domain> # マスターノードと同じである必要があります
    volumes:
      - ./data:/data
      - ./logs:/app/logs
```
...
<Callout type="warn" title="quota_data 集約キー移行">
  マルチノード構成で古いデータベースをアップグレードし、`quota_data` 集約キー移行が必要な場合は、メンテナンス時間帯に限って一時的に
  `QUOTA_DATA_AGGREGATE_MIGRATION_ENABLED=true` を設定してください。移行は
  `aggregate_key` を補完し、履歴重複集約行を統合して一意インデックスを作成します。起動成功を確認した後は、この環境変数を削除するかデフォルトの無効状態に戻すことを推奨します。
</Callout>
...
```yaml
environment:
  - LOG_SQL_DSN=root:password@tcp(log-db-host:3306)/max_api_logs # 独立したログデータベース
```
