...
このドキュメントでは、宝塔パネルのDocker機能を使用してMAX APIをデプロイするための図解チュートリアルを提供します。
...
## ステップ3：MAX APIのインストール

### 方法一：宝塔アプリストアを使用（推奨）

1. 宝塔パネルのDocker機能で、**アプリストア** をクリックします
2. **MAX API** を検索します。宝塔に MAX API の項目がまだない場合は、下記の Docker Compose またはカスタムイメージ方式で公式イメージをデプロイしてください
3. **インストール** をクリックします
4. 以下の基本オプションを設定します：
   - **コンテナ名**：カスタマイズ可能、デフォルトは `max-api`
...
1. 宝塔パネルでウェブサイトディレクトリを作成します（例：`/www/wwwroot/max-api`）
...
3. イメージ名を入力：`cscitechtop/max-api:latest`
4. **送信** をクリックし、イメージのプルが完了するまで待ちます
5. プル完了後、**コンテナリスト** に入り、**コンテナ作成** をクリックします
6. 以下の情報を入力します：
   - **コンテナ名**：`max-api`（カスタマイズ可能）
   - **イメージ**：プルした `cscitechtop/max-api:latest` を選択
