...
このドキュメントでは、Docker Compose を使用して MAX API をデプロイする詳細な手順を説明します。
...
- **指定サービスのログ**（例：`max-api`、`mysql`、`redis`）
...
- **コンテナ名を使用してログを表示**（`container_name` が設定されている場合、例：設定内の `max-api`）
