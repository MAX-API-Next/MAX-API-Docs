...
このドキュメントでは、MAX APIがサポートするすべての環境変数とその設定について説明します。これらの環境変数を設定することで、システムの動作をカスタマイズできます。

<Callout type="info" title="ヒント">
  MAX API は `.env` ファイルから環境変数を読み込むことをサポートしています。`.env.example`
...
| 環境変数                | 説明                           | デフォルト値                   | 例                                                                                                                             |
| ----------------------- | ------------------------------ | ------------------------ | -------------------------------------------------------------------------------------------------------------------------------- |
| `SQL_DSN`               | データベース接続文字列         | SQLite (`one-api.db`) | MySQL: `SQL_DSN=root:123456@tcp(localhost:3306)/max-api` \| PostgreSQL: `SQL_DSN=postgresql://root:123456@postgres:5432/max-api` |
...
| `SQLITE_PATH`           | SQLiteデータベースパス         | `one-api.db`             | `SQLITE_PATH=/var/lib/max-api/max-api.db`                                                                                        |
...
| `QUOTA_DATA_AGGREGATE_MIGRATION_ENABLED` | 起動時に `quota_data` 集約キー移行を実行します。旧バージョンからのアップグレード時に `aggregate_key` を補完し、ユーザー、ユーザー名、モデル、時間、グループ、トークン、チャネル、ノードの各次元で履歴重複行を統合し、一意な集約キーインデックスを作成するために使用します | `false` | `QUOTA_DATA_AGGREGATE_MIGRATION_ENABLED=true` |

<Callout type="warn" title="quota_data 集約キー移行">
  `QUOTA_DATA_AGGREGATE_MIGRATION_ENABLED` はデフォルトで無効です。古いデータベースをアップグレードし、
  `quota_data.aggregate_key` の修復または履歴重複集約データの統合が必要な場合のみ、一時的に
  `true` に設定してください。本番環境では事前にデータベースをバックアップし、メンテナンス時間帯に実行することを推奨します。大きな
  `quota_data` テーブルでは時間がかかる可能性があり、移行中は `quota_data` 操作ロックを使って書き込みと移行を調整します。
</Callout>
...
| `FRONTEND_BASE_URL`         | フロントエンドベースURL（スレーブノードで使用、未一致のルーティングをこのアドレスにリダイレクト） | -        | `FRONTEND_BASE_URL=https://<your-max-api-domain>` |
...
| `PYROSCOPE_APP_NAME` | レポートするアプリケーション名     | `max-api`    | `PYROSCOPE_APP_NAME=my-api`  |
...
| `HOSTNAME` | レポートするホスト名ラベル           | `max-api`    | `HOSTNAME=node-1`            |
