...
このドキュメントでは、MAX API の Docker Compose 設定オプションについて詳しく説明します。これらのオプションは、さまざまなデプロイシナリオで使用できます。

## 基本的な設定構造

Docker Compose 設定ファイル `docker-compose.yml` は、MAX API サービスとその依存サービス（MySQL、Redis など）のデプロイ方法を定義します。
...
services:
  max-api:
    ...
    volumes:
      ...
      - ./logs:/app/logs
    environment:
      - SQL_DSN=postgresql://root:123456@postgres:5432/max-api # ⚠️ 重要: 本番環境ではこのパスワードを変更してください！
      #      - SQL_DSN=root:123456@tcp(mysql:3306)/max-api  # mysql サービスを指します。MySQL を使用する場合はコメント解除してください
      - REDIS_CONN_STRING=redis://redis
      - TZ=Asia/Shanghai
      - ERROR_LOG_ENABLED=true # エラーログ記録を有効にするかどうか
      - BATCH_UPDATE_ENABLED=true # バッチ更新を有効にするかどうか
    #      - QUOTA_DATA_AGGREGATE_MIGRATION_ENABLED=true  # 古いデータベースのバックアップ後、quota_data 集約キー移行が必要な場合のみ一時的に有効化してください
...
テスト目的のみの場合は、MAX API サービス自体のみを含む以下の簡易バージョンを使用できます。
...
| パラメータ       | 説明                                                                                                 |
| ---------------- | ---------------------------------------------------------------------------------------------------- |
| `image`          | イメージ名。通常は最新バージョンを取得するために`cscitechtop/max-api:latest`を使用します。            |
...
| `environment`    | 環境変数設定。MAX API の動作を設定するために使用します。                                             |
| `depends_on`     | 依存サービス。正しい順序で起動することを保証します。                                                 |
| `healthcheck`    | ヘルスチェック設定。サービスの状態を監視するために使用します。                                       |

### 環境変数説明

MAX API は複数の環境変数設定をサポートしており、以下はよく使用されるものです。

| 環境変数            | 説明                                                 | 示例                                  |
| ------------------- | ---------------------------------------------------- | ------------------------------------- |
| `SQL_DSN`           | データベース接続文字列                               | `root:123456@tcp(mysql:3306)/max-api` |
...
| `QUOTA_DATA_AGGREGATE_MIGRATION_ENABLED` | 起動時に `quota_data` 集約キー移行を実行します。旧バージョンからのアップグレード時のみ一時的に有効化してください | `true` |
