...
- **type**: Channel identifier, used for backend routing and order placement.
  ...
  - For detailed logic, see the backend controller [controller/topup.go](https://github.com/MAX-API-Next/MAX-API/blob/main/controller/topup.go).
...
- For detailed implementation logic, see the backend controller [controller/topup.go](https://github.com/MAX-API-Next/MAX-API/blob/main/controller/topup.go).
...
Ratio settings are the core configuration of the MAX API billing system. By setting different Ratios, you can flexibly control the billing standards for various models and user groups.

### Ratio System Overview

MAX API uses a three-tier Ratio system to calculate user Quota consumption:
...
In the MAX API system, Ratios are key parameters for calculating Quota consumption. Quota is the internal billing unit of the system, and all API calls are ultimately converted into Quota points for deduction.
...
#### Audio Models (Special handling, automatically processed internally by MAX API)
...
MAX API employs a dual billing mechanism of pre-consumption and post-consumption:
