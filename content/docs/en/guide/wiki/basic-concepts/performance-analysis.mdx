...
MAX API provides two types of performance analysis capabilities:
...
export PYROSCOPE_APP_NAME=max-api
...
```bash
# Check env vars
docker exec max-api env | grep -E "PPROF|PYROSCOPE"
```
