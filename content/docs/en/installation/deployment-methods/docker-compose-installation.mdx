...
This document provides detailed steps for deploying MAX API using Docker Compose.
...
1. Create a directory for MAX API deployment:
...
- **Logs for a specific service** (Examples: `max-api`, `mysql`, `redis`)

```bash
docker compose logs -f max-api
...
```bash
docker compose logs --tail=100 max-api
```

- **View logs from a recent period**

```bash
docker compose logs --since=10m max-api
```

- **Show timestamps**

```bash
docker compose logs -f -t max-api
```
...
- **View logs using container name** (when `container_name` is set, e.g., `max-api` in the configuration)

```bash
docker logs -f max-api
```
