...
This document provides a graphical tutorial for deploying MAX API using the Baota Panel Docker feature.
...
## Step 3: Install MAX API

### Method One: Using Baota App Store (Recommended)

1. In the Baota Panel Docker feature, click **App Store**
2. Search for and find **MAX API**. If Baota does not provide a MAX API entry yet, use the Docker Compose or custom image method below to deploy the official image
3. Click **Install**
4. Configure the following basic options:
   - **Container Name**: Customizable, defaults to `max-api`
...
1. Create a website directory in Baota Panel, such as `/www/wwwroot/max-api`
...
3. Enter Image Name: `cscitechtop/max-api:latest`
...
6. Fill in the following information:
   - **Container Name**: `max-api` (customizable)
   - **Image**: Select the `cscitechtop/max-api:latest` image you just pulled
