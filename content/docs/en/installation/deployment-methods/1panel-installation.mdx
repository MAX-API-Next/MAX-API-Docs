...
This document provides a graphical tutorial for deploying MAX API using the 1Panel panel.
...
## Step 2: Install MAX API

### Search for Applications

<Callout type="info" title="Search for Applications">
  Go to the application list in the App Store, find the MAX API application under the [AI] category, and install it.
</Callout>
...
<Callout type="info" title="Install Application">
  Configure MAX API application parameters on the installation page:
</Callout>

- **Name**: The name of the MAX API application to be created
- **Version**: Select the version of MAX API
- **Port**: The service port for the MAX API application
...
<Callout type="info" title="Confirm Installation">
  Click Confirm to start the installation. The page will automatically redirect to the installed applications list. Wait for the MAX API application status to change to Started.
</Callout>
...
1. In the 1Panel application list, click the **Parameters** button for the MAX API application
...
## Step 4: Access MAX API

<Callout type="info" title="Access Application">
  - On the `Installed` page, click `Jump` to enter the MAX API WebUI page
...
2. Find the MAX API application, click the **Upgrade** button
...
- [GitHub Repository](https://github.com/MAX-API-Next/MAX-API)