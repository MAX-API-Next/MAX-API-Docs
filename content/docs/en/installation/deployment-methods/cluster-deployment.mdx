...
This document provides detailed configuration steps and best practices for MAX API cluster deployment, helping you build a highly available, load-balanced distributed system.
...
The MAX API cluster adopts a master-slave architecture design:
...
services:
  max-api-master:
    ...
    ports:
      - '3000:3000'
    environment:
      - SQL_DSN=root:password@tcp(your-db-host:3306)/max-api
      ...
      # - FRONTEND_BASE_URL=https://<your-max-api-domain> # 前端基础 URL，用于邮件通知等功能
    volumes:
      - ./data:/data
      - ./logs:/app/logs
```
...
services:
  max-api-slave:
    ...
    environment:
      - SQL_DSN=root:password@tcp(your-db-host:3306)/max-api # 与主节点相同
      ...
      - TZ=Asia/Shanghai
      # 以下是可选配置
      # - FRONTEND_BASE_URL=https://<your-max-api-domain> # 需与主节点相同
    volumes:
      - ./data:/data
      - ./logs:/app/logs
```
...
server {
    listen 80;
    server_name <your-max-api-domain>;

    location / {
        proxy_pass http://max_api_cluster;
...
<Callout type="warn" title="quota_data Aggregate-Key Migration">
  When upgrading an older database in a multi-node deployment, set
  `QUOTA_DATA_AGGREGATE_MIGRATION_ENABLED=true` only temporarily during a
  maintenance window if the `quota_data` aggregate-key migration is required.
  The migration backfills `aggregate_key`, merges historical duplicate
  aggregate rows, and creates the unique index. After the startup succeeds,
  remove this environment variable or restore the default disabled state.
</Callout>
...
```yaml
environment:
  - LOG_SQL_DSN=root:password@tcp(log-db-host:3306)/max_api_logs # Separate log database
```
