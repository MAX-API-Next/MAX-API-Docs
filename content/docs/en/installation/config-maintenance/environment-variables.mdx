...
This document provides all environment variables supported by MAX API and their configuration instructions. You can customize the system's behavior by setting these environment variables.

<Callout type="info" title="Tip">
  MAX API supports reading environment variables from `.env` files. Please refer to the `.env.example`
...
| Environment Variable          | Description                                    | Default Value             | Example                                                                                                                             |
| ----------------------------- | ---------------------------------------------- | ------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `SQL_DSN`                     | Database connection string                     | SQLite (`one-api.db`)     | MySQL: `SQL_DSN=root:123456@tcp(localhost:3306)/max-api` \| PostgreSQL: `SQL_DSN=postgresql://root:123456@postgres:5432/max-api` |
...
| `SQLITE_PATH`                 | SQLite database path                           | `one-api.db`              | `SQLITE_PATH=/var/lib/max-api/max-api.db`                                                                                        |
...
| `QUOTA_DATA_AGGREGATE_MIGRATION_ENABLED` | Run the startup `quota_data` aggregate-key migration. Use it during legacy upgrades to backfill `aggregate_key`, merge historical duplicate rows by user, username, model, hour, group, token, channel, and node dimensions, and create the unique aggregate-key index | `false` | `QUOTA_DATA_AGGREGATE_MIGRATION_ENABLED=true` |

<Callout type="warn" title="quota_data Aggregate-Key Migration">
  `QUOTA_DATA_AGGREGATE_MIGRATION_ENABLED` is disabled by default. Set it to
  `true` only temporarily when upgrading an older database that needs
  `quota_data.aggregate_key` repair or historical duplicate aggregate cleanup.
  Back up the database first and run it during a maintenance window; large
  `quota_data` tables may take time, and the migration coordinates writes
  through the `quota_data` operation lock.
</Callout>
...
| `FRONTEND_BASE_URL`               | Frontend base URL (used by slave nodes, redirects unmatched routes to this address) | -             | `FRONTEND_BASE_URL=https://<your-max-api-domain>` |
...
| `PYROSCOPE_APP_NAME`          | Application name for reporting                 | `max-api`     | `PYROSCOPE_APP_NAME=my-api`  |
...
| `HOSTNAME`                    | Hostname label for reporting                   | `max-api`     | `HOSTNAME=node-1`            |
...
# FRONTEND_BASE_URL=https://<your-max-api-domain>
