...
This document provides methods and best practices for updating the MAX API system to ensure a smooth upgrade to the latest version.
...
2.  **Check update logs**: Review the update content for the latest version on [GitHub Releases](https://github.com/MAX-API-Next/MAX-API/releases)
...
If you have deployed MAX API using a single Docker container, you can update by following these steps:
...
2.  Find the MAX API application and click the **Upgrade** button
...
2.  Find the MAX API container, click **More** -> **Recreate**
...
If you have deployed MAX API by compiling from source code, the update steps are as follows:
...
# Build the frontend first (choose default or classic as needed; default shown)
...
# Build the backend last so go:embed includes the new web/default/dist
go build -o max-api
...
```shell
# 拉取特定版本的镜像
docker pull cscitechtop/max-api:v1.x.x
