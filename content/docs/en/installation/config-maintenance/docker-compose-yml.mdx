...
This document details the Docker Compose configuration options for MAX API, applicable to various deployment scenarios.

## Basic Configuration Structure

The Docker Compose configuration file `docker-compose.yml` defines how the MAX API service and its dependent services (such as MySQL, Redis) are deployed.
...
```yaml
# MAX API Docker Compose Configuration
#
# Quick Start:
...
services:
  max-api:
    ...
    volumes:
      ...
      - ./logs:/app/logs
    environment:
      - SQL_DSN=postgresql://root:123456@postgres:5432/max-api # ⚠️ IMPORTANT: Change the password in production!
      #      - SQL_DSN=root:123456@tcp(mysql:3306)/max-api  # Point to the mysql service, uncomment if using MySQL
      - REDIS_CONN_STRING=redis://redis
      - TZ=Asia/Shanghai
      - ERROR_LOG_ENABLED=true # Whether to enable error logging
      - BATCH_UPDATE_ENABLED=true # Whether to enable batch update
    #      - QUOTA_DATA_AGGREGATE_MIGRATION_ENABLED=true  # Temporarily enable only after backing up an older database to migrate quota_data aggregate keys
...
For testing purposes, you can use the following simplified version, which only includes the MAX API service itself:
...
### MAX API Service Configuration

| Parameter        | Description                                                                    |
| ---------------- | ------------------------------------------------------------------------------ |
| `image`          | Image name, usually `cscitechtop/max-api:latest` to get the latest version      |
...
| `environment`    | Environment variable settings, used to configure MAX API behavior              |
...
MAX API supports various environment variable configurations. Here are some commonly used ones:

| Environment Variable | Description                                         | Example                               |
| -------------------- | --------------------------------------------------- | ------------------------------------- |
| `SQL_DSN`            | Database connection string                          | `root:123456@tcp(mysql:3306)/max-api` |
...
| `QUOTA_DATA_AGGREGATE_MIGRATION_ENABLED` | Run the startup `quota_data` aggregate-key migration; enable temporarily during legacy upgrades | `true` |
...
services:
  max-api-master:
    image: cscitechtop/max-api:latest
    container_name: max-api-master
    restart: always
    ports:
      - '3000:3000'
    environment:
      - SQL_DSN=root:123456@tcp(mysql:3306)/max-api
      - REDIS_CONN_STRING=redis://redis
