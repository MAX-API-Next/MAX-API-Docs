---
title: Claude Code
description: Claude Code configuration guide — use Anthropic's terminal-based coding agent with MAX API. Supports code understanding, multi-file editing, and IDE integration.
---
...
<Callout type="info" title="Set MAX API Environment Variables (Windows)">
  To allow Claude Code to connect to your relay service, you need to set multiple environment variables:
</Callout>

```powershell
[Environment]::SetEnvironmentVariable('ANTHROPIC_BASE_URL', 'https://<your-max-api-domain>', 'User')
[Environment]::SetEnvironmentVariable('ANTHROPIC_AUTH_TOKEN', 'sk-your-max-api-key', 'User')
```
...
<Callout type="info" title="Set MAX API Environment Variables">
  Enter the command:

  ```bash
  export ANTHROPIC_BASE_URL="https://<your-max-api-domain>"
  export ANTHROPIC_AUTH_TOKEN="sk-your-max-api-key"
  ```
</Callout>
