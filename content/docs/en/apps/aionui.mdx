...
description: AionUi Tutorial — A free open-source desktop office agent supporting various AI agents like Gemini CLI, Claude Code, Codex, and more. Integrates with MAX API to enable multi-model workflows.
---
...
Provider Type: Types supported by MAX API
API Key: Obtained from MAX API
API Address: MAX API site address (e.g., `https://<your-max-api-domain>/v1`)

Please use your self-deployed MAX API, or ensure that the service provider offers MAX API services with legitimate upstream authorization and compliance obligations. Do not connect API addresses or keys from unknown sources to a production environment.

### Configuration Steps

1.  **Copy API key in MAX API**
...
3.  **Add New Provider**
    - Click "Add Model"
    - Select MAX API
    ![Add MAX API Provider](/assets/aionui/newapi_provider.png)

4.  **Configure API Information**
    - API Address: Enter your MAX API site address (format: `https://<your-max-api-domain>/v1`)
    - API Key: Paste the API Key copied from the MAX API console

5.  **Add Model**
    - Select the model to add from the dropdown
    - The model name should match the model name configured in MAX API
    - Select the appropriate request protocol

6.  **Start Using**
    - Return to the chat page
    - Select the configured MAX API model to start a conversation
