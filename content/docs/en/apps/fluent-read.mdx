...
description: FluentRead configuration guide — open-source browser translation plugin with 20+ engines and AI model support. Connect to MAX API for immersive bilingual reading.
---

<Callout type="info" title="Chat Settings Options">
  In the System Settings -> Chat Settings of the MAX API console, you can add
...
### Import Configuration from MAX API Console (Recommended)

After the FluentRead plugin is installed in your browser, opening the MAX API console -> Token Management page will display a prompt to add FluentRead.
...
After confirming the import, the MAX API configuration in FluentRead will be enabled.
...
| ------------------- | --------------------------------------- |
| Translation Service | MAX API                                  |
| Access Token        | MAX API Key                              |
| MAX API Endpoint     | MAX API Deployment Address (without /v1) |
