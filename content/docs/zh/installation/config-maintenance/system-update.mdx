...
本文档提供了MAX API系统的更新方法和最佳实践，以确保您的系统平稳升级到最新版本。
...
1. **备份数据**：备份数据库和重要配置文件
2. **查看更新日志**：在[GitHub Releases](https://github.com/MAX-API-Next/MAX-API/releases)查看最新版本的更新内容
...
如果您使用单个Docker容器部署了MAX API，可以按照以下步骤更新：
...
# 最后编译后端，使 go:embed 嵌入刚生成的 web/default/dist
go build -o max-api
...
# 最后重新编译后端，使 go:embed 嵌入刚生成的 web/default/dist
go build -o max-api
