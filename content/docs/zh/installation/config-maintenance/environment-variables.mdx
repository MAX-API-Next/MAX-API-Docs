...
本文档提供了MAX API支持的所有环境变量及其配置说明。您可以通过设置这些环境变量来自定义系统的行为。

<Callout type="info" title="提示">
  MAX API 支持从 `.env` 文件中读取环境变量，请参照 `.env.example`
  文件，使用时请将其重命名为 `.env`。
</Callout>
...
| 环境变量                | 说明                       | 默认值                   | 示例                                                                                                                             |
| ----------------------- | -------------------------- | ------------------------ | -------------------------------------------------------------------------------------------------------------------------------- |
| `SQL_DSN`               | 数据库连接字符串           | SQLite (`one-api.db`) | MySQL: `SQL_DSN=root:123456@tcp(localhost:3306)/max-api` \| PostgreSQL: `SQL_DSN=postgresql://root:123456@postgres:5432/max-api` |
...
| `SQLITE_PATH`           | SQLite数据库路径           | `one-api.db`             | `SQLITE_PATH=/data/one-api.db`                                                                                                   |
...
| `QUOTA_DATA_AGGREGATE_MIGRATION_ENABLED` | 启动时执行 `quota_data` 聚合键迁移。用于升级旧库时回填 `aggregate_key`，按用户、用户名、模型、小时、分组、令牌、渠道、节点维度合并历史重复行，并创建唯一聚合键索引 | `false` | `QUOTA_DATA_AGGREGATE_MIGRATION_ENABLED=true` |

<Callout type="warn" title="quota_data 聚合键迁移">
  `QUOTA_DATA_AGGREGATE_MIGRATION_ENABLED` 默认关闭。仅在升级旧版本数据库、确认需要修复
  `quota_data.aggregate_key` 或合并历史重复聚合数据时临时设置为 `true`。生产环境建议先备份数据库，并在维护窗口执行；大表迁移可能耗时，迁移期间会通过
  `quota_data` 操作锁协调写入与迁移。
</Callout>
...
| `FRONTEND_BASE_URL`         | 前端基础URL（从节点使用，将未匹配路由重定向到此地址） | -        | `FRONTEND_BASE_URL=https://<your-max-api-domain>` |
...
| `PYROSCOPE_APP_NAME` | 上报的应用名称 | `max-api` | `PYROSCOPE_APP_NAME=max-api` |
...
| `HOSTNAME` | 上报的主机名标签 | `max-api` | `HOSTNAME=max-api-node-1` |
...
```bash
# 数据库配置 - 使用远程数据库
SQL_DSN=root:password@tcp(db-server:3306)/max-api
