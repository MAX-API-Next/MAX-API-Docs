...
本文档提供了在本地环境中设置和开发 MAX API 项目的详细步骤，适合希望参与项目开发或进行功能定制的开发者。

## 开发环境要求

在开始本地开发之前，请确保您的系统已安装以下软件：

- **Go** 1.25.1 或更高版本（后端开发）
...
MAX API 支持通过 `.env` 文件配置环境变量。创建 `.env` 文件（可从 `.env.example` 复制）：
...
```bash
PORT=3000
SQL_DSN=root:password@tcp(localhost:3306)/max-api   # 如使用MySQL，取消注释并修改
...
MAX API 的前端代码位于 `web` 目录中，使用 React 和 [semi design 组件库](https://semi.design/zh-CN) 开发。
...
│  main.go                               # 项目主入口文件
│  makefile                              # 项目构建脚本
│  max-api.service                       # systemd 服务配置文件
...
1. 查看 [GitHub Issues](https://github.com/MAX-API-Next/MAX-API/issues)
