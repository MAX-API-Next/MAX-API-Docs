...
本文档提供了 MAX API 集群部署的详细配置步骤和最佳实践，帮助您构建高可用、负载均衡的分布式系统。
...
4. **设置稳定节点名称**：多节点建议设置 `NODE_NAME`，便于日志定位和异步任务结算归属
...
services:
  max-api-slave:
    ...
    environment:
      ...
      - NODE_NAME=max-api-slave # 稳定节点名，便于日志和任务结算定位
...
<Callout type="warn" title="quota_data 聚合键迁移">
  多节点升级旧版本数据库时，如需执行 `quota_data` 聚合键迁移，可在维护窗口临时设置
  `QUOTA_DATA_AGGREGATE_MIGRATION_ENABLED=true`。迁移会回填 `aggregate_key`、合并历史重复聚合行并创建唯一索引；完成并确认启动成功后，建议移除该环境变量或恢复为默认关闭。
</Callout>
...
```yaml
environment:
  - LOG_SQL_DSN=root:password@tcp(log-db-host:3306)/max_api_logs # 独立的日志数据库
```
