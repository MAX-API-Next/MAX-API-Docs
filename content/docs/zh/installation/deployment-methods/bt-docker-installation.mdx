...
2. 搜索并找到 **MAX API**；如果应用商店尚未提供 MAX API 条目，请使用下方 Docker Compose 或自定义镜像方式部署官方镜像
3. 点击 **安装**
4. 配置以下基本选项：
   - **容器名称**：可自定义，建议使用 `max-api`
   - **端口映射**：默认为 `3000:3000`
   - **环境变量**：
...
2. 点击 **获取镜像** → **拉取镜像**
3. 输入镜像名称：`cscitechtop/max-api:latest`
4. 点击 **提交**，等待镜像拉取完成
5. 拉取完成后，进入 **容器列表**，点击 **创建容器**
6. 填写以下信息：
   - **容器名称**：`max-api`（可自定义）
   - **镜像**：选择刚刚拉取的 `cscitechtop/max-api:latest`
   - **端口映射**：添加 `3000:3000`
